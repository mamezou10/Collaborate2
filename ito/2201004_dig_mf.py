...
out_dir = '/mnt/Donald/ito/221004_5'
#os.chdir(out_dir)
...
np.random.seed(1213)
torch.manual_seed(1)
os.environ['PYTHONHASHSEED'] = '1213'
...
sc.tl.leiden(est_adata_tmp, key_added="fine_leiden", resolution=0.9)
...
msig_gmt = commons.parse_gmt_500('gmts/c5.go.bp.v7.5.1.symbols.gmt')
...
for i in range(10):
    ...
    deg_df = pd.merge(deg_df, mouse2human, left_on="names", right_on="mouse")
    # cl_genes = deg_df[(deg_df.scores>10)].sort_values("scores", ascending=False).human[:500]
    # cl_genes = deg_df[(deg_df.scores>1) & (deg_df.pvals_adj<10**(-10))]
    deg_df.sort_values("scores", ascending=False).to_csv(f'{out_dir}/cl_genes_clu{targ_fine_cluster}.csv')
    cl_genes = deg_df.sort_values("scores", ascending=False).query("scores > 5 & pvals_adj < 10**-4 ").human
    cl_genes = cl_genes[:300]
    print(len(cl_genes))
    ...
    df_bar = df_bar[(df_bar["gene_set"].str.contains("^GOBP")) & (df_bar["gene_set"].str.contains("IMMUN")) & (df_bar["padj"] < 0.05)]
