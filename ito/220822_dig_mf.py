...
out_dir = '/mnt/Donald/ito/220822_mf'
#os.chdir(out_dir)
...
deg_df = pd.DataFrame({
    key: est_adata_tmp.uns['wilcoxon'][key]["8"]
    for key in keys
}, index=est_adata_tmp.uns['wilcoxon']['names']["8"])

deg_df = pd.merge(deg_df, mouse2human, left_on="names", right_on="mouse")
cl_genes = deg_df.query("scores > 12 & pvals_adj < 10**-4 ").human
total_genes = pd.merge(pd.DataFrame(est_adata.var_names), mouse2human, left_on="Gene", right_on="mouse").human
pval_df = commons.gene_set_enrichment(cl_genes, total_genes, impla_gmt)
pval_df.to_csv(f'{out_dir}/enrich_impla_cl8_2.csv')
pval_df = commons.gene_set_enrichment(cl_genes, total_genes, msig_gmt)
pval_df.to_csv(f'{out_dir}/enrich_msig_cl8_2.csv')

# pattern2 vs cdiff_cl2
pval_df = commons.gene_set_enrichment(cl_genes, cdiff_cl2_genes, impla_gmt)
pval_df.to_csv(f'{out_dir}/enrich_impla_cdiffclu2_cl8.csv')
pval_df = commons.gene_set_enrichment(cl_genes, cdiff_cl2_genes, msig_gmt)
pval_df.to_csv(f'{out_dir}/enrich_msig_cdiffclu2_cl8.csv')



#cl9
deg_df = pd.DataFrame({
    key: est_adata_tmp.uns['wilcoxon'][key]["9"]
    for key in keys
}, index=est_adata_tmp.uns['wilcoxon']['names']["9"])

deg_df = pd.merge(deg_df, mouse2human, left_on="names", right_on="mouse")
cl_genes = deg_df.query("scores > 0 & pvals_adj < 10**-4 ").human
pval_df = commons.gene_set_enrichment(cl_genes, total_genes, impla_gmt)
pval_df.to_csv(f'{out_dir}/enrich_impla_cl9_2.csv')
pval_df = commons.gene_set_enrichment(cl_genes, total_genes, msig_gmt)
pval_df.to_csv(f'{out_dir}/enrich_msig_cl9_2.csv')

# pattern2 vs cdiff_cl2
pval_df = commons.gene_set_enrichment(cl_genes, cdiff_cl2_genes, impla_gmt)
pval_df.to_csv(f'{out_dir}/enrich_impla_cdiffclu2_cl9.csv')
pval_df = commons.gene_set_enrichment(cl_genes, cdiff_cl2_genes, msig_gmt)
pval_df.to_csv(f'{out_dir}/enrich_msig_cdiffclu2_cl9.csv')


#cl2
deg_df = pd.DataFrame({
    key: est_adata_tmp.uns['wilcoxon'][key]["2"]
    for key in keys
}, index=est_adata_tmp.uns['wilcoxon']['names']["2"])

deg_df = pd.merge(deg_df, mouse2human, left_on="names", right_on="mouse")
cl_genes = deg_df.query("scores > 7 & pvals_adj < 10**-4 ").human
pval_df = commons.gene_set_enrichment(cl_genes, total_genes, impla_gmt)
pval_df.to_csv(f'{out_dir}/enrich_impla_cl2_2.csv')
pval_df = commons.gene_set_enrichment(cl_genes, total_genes, msig_gmt)
pval_df.to_csv(f'{out_dir}/enrich_msig_cl2_2.csv')

# pattern2 vs cdiff_cl2
total = pd.merge(cdiff_cl2_genes, cl_genes, how="outer").drop_duplicates()
pval_df = commons.gene_set_enrichment(cl_genes, total["human"], impla_gmt)
pval_df.to_csv(f'{out_dir}/enrich_impla_cdiffclu2_cl2.csv')
pval_df = commons.gene_set_enrichment(cl_genes, total["human"], msig_gmt)
pval_df.to_csv(f'{out_dir}/enrich_msig_cdiffclu2_cl2.csv')



#cl3
deg_df = pd.DataFrame({
    key: est_adata_tmp.uns['wilcoxon'][key]["3"]
    for key in keys
}, index=est_adata_tmp.uns['wilcoxon']['names']["3"])

deg_df = pd.merge(deg_df, mouse2human, left_on="names", right_on="mouse")
cl_genes = deg_df.query("scores > 0 & pvals_adj < 10**-4 ").human
pval_df = commons.gene_set_enrichment(cl_genes, total_genes, impla_gmt)
pval_df.to_csv(f'{out_dir}/enrich_impla_cl3_2.csv')
pval_df = commons.gene_set_enrichment(cl_genes, total_genes, msig_gmt)
pval_df.to_csv(f'{out_dir}/enrich_msig_cl3_2.csv')

# pattern2 vs cdiff_cl2
pval_df = commons.gene_set_enrichment(cl_genes, cdiff_cl2_genes, impla_gmt)
pval_df.to_csv(f'{out_dir}/enrich_impla_cdiffclu2_cl3.csv')
pval_df = commons.gene_set_enrichment(cl_genes, cdiff_cl2_genes, msig_gmt)
pval_df.to_csv(f'{out_dir}/enrich_msig_cdiffclu2_cl3.csv')


#cl1
deg_df = pd.DataFrame({
    key: est_adata_tmp.uns['wilcoxon'][key]["1"]
    for key in keys
}, index=est_adata_tmp.uns['wilcoxon']['names']["1"])

deg_df = pd.merge(deg_df, mouse2human, left_on="names", right_on="mouse")
cl_genes = deg_df.query("scores > 20 & pvals_adj < 10**-4 ").human
pval_df = commons.gene_set_enrichment(cl_genes, total_genes, impla_gmt)
pval_df.to_csv(f'{out_dir}/enrich_impla_cl1_2.csv')
pval_df = commons.gene_set_enrichment(cl_genes, total_genes, msig_gmt)
pval_df.to_csv(f'{out_dir}/enrich_msig_cl1_2.csv')

# pattern2 vs cdiff_cl2
pval_df = commons.gene_set_enrichment(cl_genes, cdiff_cl2_genes, impla_gmt)
pval_df.to_csv(f'{out_dir}/enrich_impla_cdiffclu2_cl1.csv')
pval_df = commons.gene_set_enrichment(cl_genes, cdiff_cl2_genes, msig_gmt)
pval_df.to_csv(f'{out_dir}/enrich_msig_cdiffclu2_cl1.csv')


#cl0
deg_df = pd.DataFrame({
    key: est_adata_tmp.uns['wilcoxon'][key]["0"]
    for key in keys
}, index=est_adata_tmp.uns['wilcoxon']['names']["0"])

deg_df = pd.merge(deg_df, mouse2human, left_on="names", right_on="mouse")
cl_genes = deg_df.query("scores > 30 & pvals_adj < 10**-4 ").human
pval_df = commons.gene_set_enrichment(cl_genes, total_genes, impla_gmt)
pval_df.to_csv(f'{out_dir}/enrich_impla_cl0_2.csv')
pval_df = commons.gene_set_enrichment(cl_genes, total_genes, msig_gmt)
pval_df.to_csv(f'{out_dir}/enrich_msig_cl0_2.csv')

# pattern2 vs cdiff_cl2
pval_df = commons.gene_set_enrichment(cl_genes, cdiff_cl2_genes, impla_gmt)
pval_df.to_csv(f'{out_dir}/enrich_impla_cdiffclu2_cl0.csv')
pval_df = commons.gene_set_enrichment(cl_genes, cdiff_cl2_genes, msig_gmt)
pval_df.to_csv(f'{out_dir}/enrich_msig_cdiffclu2_cl0.csv')


#cl5
deg_df = pd.DataFrame({
    key: est_adata_tmp.uns['wilcoxon'][key]["5"]
    for key in keys
}, index=est_adata_tmp.uns['wilcoxon']['names']["5"])

deg_df = pd.merge(deg_df, mouse2human, left_on="names", right_on="mouse")
cl_genes = deg_df.query("scores > 5.5 & pvals_adj < 10**-4 ").human
pval_df = commons.gene_set_enrichment(cl_genes, total_genes, impla_gmt)
pval_df.to_csv(f'{out_dir}/enrich_impla_cl5_2.csv')
pval_df = commons.gene_set_enrichment(cl_genes, total_genes, msig_gmt)
pval_df.to_csv(f'{out_dir}/enrich_msig_cl5_2.csv')

# pattern2 vs cdiff_cl2
total = pd.merge(cdiff_cl2_genes, cl_genes, how="outer").drop_duplicates()
pval_df = commons.gene_set_enrichment(cl_genes, total["human"], impla_gmt)
pval_df.to_csv(f'{out_dir}/enrich_impla_cdiffclu2_cl5.csv')
pval_df = commons.gene_set_enrichment(cl_genes, total["human"], msig_gmt)
pval_df.to_csv(f'{out_dir}/enrich_msig_cdiffclu2_cl5.csv')



#cl6
deg_df = pd.DataFrame({
    key: est_adata_tmp.uns['wilcoxon'][key]["6"]
    for key in keys
}, index=est_adata_tmp.uns['wilcoxon']['names']["6"])

deg_df = pd.merge(deg_df, mouse2human, left_on="names", right_on="mouse")
cl_genes = deg_df.query("scores > 8 & pvals_adj < 10**-4 ").human
pval_df = commons.gene_set_enrichment(cl_genes, total_genes, impla_gmt)
pval_df.to_csv(f'{out_dir}/enrich_impla_cl6_2.csv')
pval_df = commons.gene_set_enrichment(cl_genes, total_genes, msig_gmt)
pval_df.to_csv(f'{out_dir}/enrich_msig_cl6_2.csv')

# pattern2 vs cdiff_cl2
total = pd.merge(cdiff_cl2_genes, cl_genes, how="outer").drop_duplicates()
pval_df = commons.gene_set_enrichment(cl_genes, total["human"], impla_gmt)
pval_df.to_csv(f'{out_dir}/enrich_impla_cdiffclu2_cl6.csv')
pval_df = commons.gene_set_enrichment(cl_genes, total["human"], msig_gmt)
pval_df.to_csv(f'{out_dir}/enrich_msig_cdiffclu2_cl6.csv')


## fig from pattern2
msig5 = pd.read_csv(f'{out_dir}/enrich_msig_cdiffclu2_cl5.csv')
msig6 = pd.read_csv(f'{out_dir}/enrich_msig_cdiffclu2_cl6.csv')
...
cl = ["c", "r", "c", "r", "c"]
sns.barplot(data=df_bar[df_bar["group"]=="clu5"], y='gene_set', x='-log10_padj', palette=cl)
# plt.xscale("log")
plt.tick_params(labelsize=10)
plt.subplots_adjust(left=0.7)
plt.savefig(f'{out_dir}/cdiff2_clu5_GO.png');plt.close('all')

plt.figure(figsize=(15,10))
cl = ["c", "c", "c", "r", "r", "c", "c", "gray", "gray", "c", "gray", "c"]
sns.barplot(data=df_bar[df_bar["group"]=="clu6"], y='gene_set', x='-log10_padj', palette=cl)
# plt.xscale("log")
plt.tick_params(labelsize=10)
plt.subplots_adjust(left=0.7)
plt.savefig(f'{out_dir}/cdiff2_clu6_GO.png');plt.close('all')
...
# sc.pl.rank_genes_groups_matrixplot(est_adata_tmp, n_genes=5, key="cdiff2_and_fineClusters_wilcoxon", groupby="cdiff_cluster", show=False)
# plt.savefig(f'{out_dir}/cdiff_heatmap.png');plt.close('all')
...
import seaborn as sns
plt.figure(figsize=(15,5))
cl = ["gray","gray","c","c","c", "gray","gray","r", "gray","c", "c","r", "c", "c", "c"]
sns.barplot(data=df_bar[df_bar["group"]=="clu5"], y='gene_set', x='-log10_padj', palette=cl)
# plt.xscale("log")
plt.tick_params(labelsize=10)
plt.subplots_adjust(left=0.7)
plt.savefig(f'{out_dir}/cdiff2_direct_clu5_GO.png');plt.close('all')

plt.figure(figsize=(15,30))
cl = ["gray" for i in range(len(df_bar[df_bar["group"]=="clu6"]))]
for i in [2,6,7,8,12,14,15,17,19,20,22,30,32,34,35,37,44,52,58,59,63,65,66,68,72,74,90,92,94,100,106,111,113]:
    cl[i] = "c"
...
plt.tick_params(labelsize=10)
plt.subplots_adjust(left=0.7)
plt.savefig(f'{out_dir}/cdiff2_direct_clu6_GO.png');plt.close('all')
...
msig1 = pd.read_csv(f'{out_dir}/enrich_msig_cl1.csv')
msig3 = pd.read_csv(f'{out_dir}/enrich_msig_cl3.csv')
msig8 = pd.read_csv(f'{out_dir}/enrich_msig_cl8.csv')
msig9 = pd.read_csv(f'{out_dir}/enrich_msig_cl9.csv')
msig1.columns = ["gene_set", "padj"]
msig3.columns = ["gene_set", "padj"]
msig8.columns = ["gene_set", "padj"]
msig9.columns = ["gene_set", "padj"]


msig1 = msig1[(msig1["gene_set"].str.contains("IMMUN" or "INFLA")) & (msig1.padj < 0.05)]
msig3 = msig3[(msig3["gene_set"].str.contains("IMMUN" or "INFLA")) & (msig3.padj < 0.05)]
msig8 = msig8[(msig8["gene_set"].str.contains("IMMUN" or "INFLA")) & (msig8.padj < 0.05)]
msig9 = msig9[(msig9["gene_set"].str.contains("IMMUN" or "INFLA")) & (msig9.padj < 0.05)]

df_bar = pd.DataFrame({
    "group": ["clu1" for i in range(len(msig1))] + ["clu3" for i in range(len(msig3))] + ["clu8" for i in range(len(msig8))] + ["clu9" for i in range(len(msig9))],
    "gene_set": list(msig1.gene_set) + list(msig3.gene_set) + list(msig8.gene_set) + list(msig9.gene_set) ,
    "padj": list(msig1.padj) + list(msig3.padj) + list(msig8.padj) + list(msig9.padj) ,
})
df_bar["-log10_padj"] = -np.log10(df_bar.padj)

df_bar = df_bar[df_bar["gene_set"].str.contains("GOBP_")]

# df = pd.concat([
#     df_bar[~(df_bar.gene_set.str.contains("NEGATIVE")) & (df_bar.group != "clu3")].sort_values("-log10_padj", ascending=False),
#     df_bar[(df_bar.gene_set.str.contains("NEGATIVE")) & (df_bar.group != "clu3")].sort_values("-log10_padj", ascending=False)],
#     axis=0)

df_bar = df_bar[(df_bar.group=="clu8")|(df_bar.group=="clu9")]
df = df_bar[(df_bar.gene_set.str.contains("NEGATIVE")) ].sort_values("-log10_padj", ascending=False)

import seaborn as sns
plt.figure(figsize=(15,10))
sns.barplot(data=df, y='gene_set', x='-log10_padj', hue='group')
# plt.xscale("log")
plt.tick_params(labelsize=10)
plt.subplots_adjust(left=0.5)
plt.savefig(f'{out_dir}/clu8_9_neg_bar.png');plt.close('all')

df = df_bar[(df_bar.gene_set.str.contains("POSITIVE")) ].sort_values("-log10_padj", ascending=False)
plt.figure(figsize=(15,10))
sns.barplot(data=df, y='gene_set', x='-log10_padj', hue='group')
# plt.xscale("log")
plt.tick_params(labelsize=10)
plt.subplots_adjust(left=0.5)
plt.savefig(f'{out_dir}/clu8_9_pos_bar.png');plt.close('all')




## misc
stats.spearmanr(cdiff_adata[intersect_cells, : ].obs["cond2_ratio"], est_adata[intersect_cells, "Ccr5" ].layers["lambda"]).correlation
sc.pl.umap(est_adata_tmp, color=['Ccr5', 'Cast', 'Klf7', 'Ccl6'], show=True, layer="lambda")
sc.pl.umap(est_adata, color=['Ccr5'], show=True, layer="lambda")
sc.pl.umap(cdiff_adata, color=["cond2_ratio"], show=True)
plt.scatter(cluster_adata.obs.cond2_ratio, est_adata_tmp[source_cells, "Ccr5"].layers["lambda"])
