...
# home_dir = '/Users/kojimayasuhiro/Dropbox/Projects/envdynamics/'
# base_dir = home_dir + "envdyn_eperiment/"
# envdyn_dir = base_dir + "envdyn"
# sys.path.append(envdyn_dir)
...
# root_dir = "inc_transient_2021_01_05/"


# def define_sim_exp_no_env(
#         data_set_params = {
#             'root': root_dir, 
#             'read_pos': np.loadtxt, 'read_splice': lambda x: torch.tensor(np.loadtxt(x).transpose()), 
#             'read_unsplice': lambda x: torch.tensor(np.loadtxt(x).transpose()), 
#             'read_meta': lambda x: pd.read_csv(x), 
#             'raw_file_names': ['splice_mat_2021_01_05.txt', 'unsplice_mat_2021_01_05.txt', 'meta_2021_01_05.csv']},
#         model_params = {
#             'x_dim': 100,
#             'z_dim': 10,
...
#             'num_dec_z_layers': 2    
#         },
#         lr=0.01, num_parts=20, val_ratio=0.05, test_ratio=0.1,
#         batch_size=100, num_workers=1, x_dim=100):
#     model_params['x_dim'] = x_dim
#     s_ident, u_ident, meta_ident = data_set_params['raw_file_names']
#     data_dir = data_set_params['root'] + 'raw/'
#     # splice
#     s_fname = data_dir + s_ident
#     s = data_set_params['read_splice'](s_fname)
#     # unspliced
#     u_fname = data_dir + u_ident
#     u = data_set_params['read_unsplice'](u_fname)
#     # meta df
#     meta_fname = data_dir + meta_ident
#     meta_df = data_set_params['read_meta'](meta_fname)
#     envdyn_exp = envdyn.EnvDynExperiment(model_params, lr, s, u, meta_df, test_ratio, batch_size, num_workers)
#     return(envdyn_exp)


def define_scv_exp_no_env(
        s_fname, u_fname, meta_fname,
        model_params = {
            'x_dim': 100,
            'z_dim': 10,
            'enc_z_h_dim': 50, 'enc_d_h_dim': 50, 'dec_z_h_dim': 50,
        ...
        batch_size=300, num_workers=1, x_dim=100):
    # splice
    s = torch.tensor(np.loadtxt(s_fname))
    select_idx = torch.sum(s, axis=0) > 100
    s = s[:, select_idx]
    # unspliced
    u = torch.tensor(np.loadtxt(u_fname))
    u = u[:, select_idx]
    # meta df
...
def define_exp(
        ...
        lr=0.0001, num_parts=20, val_ratio=0, test_ratio=0.05,
        batch_size=300, num_workers=1, min_count=50):
    # splice
    s = torch.tensor(np.loadtxt(s_fname))
    select_idx = torch.sum(s, axis=0) > min_count
    s = s[:, select_idx]
    # unspliced
    u = torch.tensor(np.loadtxt(u_fname))
    u = u[:, select_idx]
    # meta df
    model_params['x_dim'] = s.shape[1]
    envdyn_exp = envdyn.EnvDynExperiment(model_params, lr, s, u, test_ratio, batch_size, num_workers, validation_ratio=val_ratio)
    return(envdyn_exp)


def optimize_and_draw_loss_curve(envdyn_exp, epoch, fig_path, param_path):
    train_loss_list = []
    test_loss_list = []
    for epoch in range(epoch):
        test_loss = envdyn_exp.test()
        test_loss_list.append(test_loss)
        train_loss_list.append(envdyn_exp.train_epoch())
        plot_df = pd.concat(
            [
                pd.DataFrame({"# of iteration": np.arange(len(train_loss_list)), "Loss": train_loss_list, "Data set": "Train"}),
                pd.DataFrame({"# of iteration": np.arange(len(test_loss_list)), "Loss": test_loss_list, "Data set": "Test"})
            ])
        if epoch % 10 == 0:
                print(f'loss at epoch {epoch} is {test_loss}')
    plot_df["Loss value"] = [float(loss) for loss in plot_df.Loss]
    fig, axes = plt.subplots(1, 1, figsize=(6, 5 * 1))
    sns.lineplot(x="# of iteration", y="Loss value", hue="Data set", data=plot_df, ax=axes)
    plt.savefig(fig_path)
    torch.save(envdyn_exp.model.state_dict(), param_path)
    return(envdyn_exp)


def load_existing_eval(envdyn_exp, model_param_path):
...
def modify_save(axes, fig_name):
    axes.axis('off')
    fig = axes.figure
    fig.set_size_inches(10 * 0.7, 8 * 0.7)
    fig.subplots_adjust(left=0.01, bottom=0.01, right=0.7, top=7/8, wspace=0.4, hspace=0.8)
    plt.savefig(fig_name);plt.close('all')

def plot_mean_flow(adata, cluster_key='leiden', legend_loc='on data'):
    m_coeff = 0.5 
    adata.obsm['X_vicdyf_mdumap'] = adata.obsm['X_vicdyf_mdumap'] / np.linalg.norm(adata.obsm['X_vicdyf_mdumap'], axis=1, keepdims=True)
    arrow_ratio = 10
    scv.pl.velocity_embedding_grid(adata, X=adata.obsm['X_vicdyf_umap'], V=adata.obsm['X_vicdyf_mdumap'] * m_coeff, color=cluster_key, show=False, basis='X_vicdyf_umap', density=0.6, headwidth=arrow_ratio, headlength=arrow_ratio, headaxislength=arrow_ratio, width=0.002, arrow_length=3, arrow_color='black', alpha=0.8, legend_loc=legend_loc)