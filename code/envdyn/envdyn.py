...
from dataset import EnvDynDataSet, EnvDynDataManager
from torch.utils.data import DataLoader
import numpy as np
import copy

class EnvDynExperiment:
    def __init__(self, model_params, lr, s, u, test_ratio, batch_size, num_workers, validation_ratio=0.1):
        self.edm = EnvDynDataManager(s, u, test_ratio, batch_size, num_workers, validation_ratio=validation_ratio)
        self.device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        self.model = EnvDyn(**model_params)
