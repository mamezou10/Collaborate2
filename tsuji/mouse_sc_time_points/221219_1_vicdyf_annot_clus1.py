...
import workflow
import condiff
sys.path.append("/home/hirose/Documents/main/Collaborate2/code")
from misc import *
...
for term in pre_res.res2d.sort_values('pval').head(n=10).index:
    ...
    gseapy.gseaplot(rank_metric=pre_res.ranking,
         ...
         **pre_res.results[term],
         ofname=f'{out_dir}/gsea_{cluster}/{term[:3]}_gsea.png')


## それをGSEA msig human
pre_res = gseapy.prerank(cdiff_df, gene_sets='/home/hirose/Documents/main/gmts/msigdb.v7.4.symbols.gmt', outdir=f'{out_dir}/gsea_msig_human_{cluster}', graph_num=10, permutation_num=1000, format='png', seed=6, processes=12)
...
for term in pre_res.res2d.sort_values('pval').head(n=10).index:
    ...
    gseapy.gseaplot(rank_metric=pre_res.ranking,
         term=term,
         **pre_res.results[term],
         ofname=f'{out_dir}/gsea_msig_human_{cluster}/{term[:3]}_gsea.png')



## それをGSEA msig mouse
cdiff_df_mouse = pd.DataFrame({'gene': deg_cdiff_df.names, 'scores': deg_cdiff_df.score}).dropna().sort_values("scores", ascending=False)
pre_res = gseapy.prerank(cdiff_df_mouse, gene_sets='/home/hirose/Documents/main/gmts/msigdb.v2022.1.Mm.symbols.gmt', outdir=f'{out_dir}/gsea_msig_{cluster}', graph_num=10, permutation_num=1000, format='png', seed=6, processes=12)
...
for term in pre_res.res2d.sort_values('pval').head(n=10).index:
    ...
    gseapy.gseaplot(rank_metric=pre_res.ranking,
         term=term,
         **pre_res.results[term],
         ofname=f'{out_dir}/gsea_msig_{cluster}/{term[:3]}_gsea.png')
...
for gene in lt_df.columns:
    targ_genes = list(lt_df.sort_values(gene, ascending=False)[[gene]][:300].index)
    targ_genes.insert(0,str(len(targ_genes)))
    targ_genes.insert(0,gene)
    targ_genes_list.append("\t".join(targ_genes))

pd.Series(targ_genes_list).to_csv(f'{out_dir}/targ_gmt.gmt', header=False, index=False)
# pd.Series(targ_genes_list[185:190]).to_csv(f'{out_dir}/targ_gmt2.gmt', header=False, index=False)

pre_res = gseapy.prerank(cdiff_df, gene_sets=f'{out_dir}/targ_gmt.gmt', outdir=f'{out_dir}/gsea_lt_{cluster}', graph_num=10, permutation_num=1000, format='png', seed=6, processes=12)
