...
wd = "/mnt/Bambi/Projects/morim_resist/analysis/220805_pattern3/"
os.makedirs(wd, exist_ok=True)
os.chdir(wd)

## MUC1>0を元データから取得
raw_adata = sc.read_10x_h5("/mnt/Bambi/scCRC_GSE178341/GSE178341_crc10x_full_c295v4_submit.h5")
bool = pd.DataFrame.sparse.from_spmatrix(raw_adata[:,raw_adata.var_names=="MUC1"].X)>0
MUC1pos_cells = pd.DataFrame(raw_adata.obs_names)[bool.values.reshape(-1)].values.reshape(-1).tolist()

bool = pd.DataFrame.sparse.from_spmatrix(raw_adata[:,raw_adata.var_names=="MICA"].X)>0
MICApos_cells = pd.DataFrame(raw_adata.obs_names)[bool.values.reshape(-1)].values.reshape(-1).tolist()


# pattern3のpreprocessed data
adata = sc.read_h5ad("/mnt/Bambi/scCRC_GSE178341/scCRC.h5ad")
sc.pl.umap(adata, color="clTopLevel", legend_loc="on data", save="umap.pdf")
