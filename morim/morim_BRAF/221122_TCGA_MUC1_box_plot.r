...
g <-  ggplot(df[df$BRAF_mut!="NA"&df$BRAF_mut!="none", ], aes(x = BRAF_mut, y = MUC1, fill=BRAF_mut)) + 
        ...
        geom_signif(comparisons = list(c("p.V600E", "WT")), map_signif_level=TRUE, textsize=9, family="serif", y_position = 17000, vjust = 0.2) +#, test = "t.test") 
        geom_signif(comparisons = list(c("p.D594N", "WT")), y_position = 19000) 
