...
thre_half <- quantile(exp$PTPN11, c(0,0.5,1.0), na.rm=TRUE)

exp$PTPN11_half <- cut(exp$PTPN11, thre_half,
                labels = c("low","high"),
                include.lowest = TRUE)
...
# KM WT
sfit <- survfit(Surv(days, event) ~ PTPN11_half, data=df_surv[df_surv$BRAF_mut=="WT",])
summary(sfit)
g <- ggsurvplot(sfit, conf.int=FALSE, pval=TRUE, risk.table=TRUE, xscale = "d_m" ,break.time.by=365.25*0.5,
           legend.labs=c("low", "high"), legend.title="PTPN11",  xlab="month",
           palette=c("dodgerblue2", "orchid2"), 
           title="Kaplan-Meier Curve for COAD", 
           risk.table.height=.3)

png("survival_PTPN11_wt_final_no_censor.png")
print(g, newpage = FALSE)
dev.off()


# KM mut
sfit <- survfit(Surv(days, event) ~ PTPN11_half, data=df_surv[df_surv$BRAF_mut=="p.V600E",])
summary(sfit)
...
png("survival_PTPN11_mt_final_no_censor.png")
print(g, newpage = FALSE)
dev.off()
