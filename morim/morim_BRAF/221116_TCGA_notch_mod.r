...
g <-  ggplot(df[df$BRAF_mut!="NA"&df$BRAF_mut!="none", ], aes(x = BRAF_mut, y = MUC1, fill=BRAF_mut)) + 
        geom_boxplot() +
...
df_surv_censor <- df_surv %>% 
                mutate(vital_status = ifelse(days_to_death > 1825, "Alive", vital_status)) %>%
                mutate(days_to_death = ifelse(days_to_death > 1825, 1825, days_to_death))  %>%
                mutate(days_to_last_follow_up = ifelse(days_to_last_follow_up > 1825, 1825, days_to_last_follow_up)) %>%  
                mutate(days = ifelse(days > 1825, 1825, days))  

TCGAanalyze_survival(df_surv_censor[df_surv_censor$BRAF_mut=="WT",], "NOTCH_half", risk.table=FALSE, 
        main = "TCGA Set\n NOTCH_q\n BRAF mutatnt",height = 10, width=10, filename = "survival_NOTCH_half_wt_5yrs.png")

TCGAanalyze_survival(df_surv_censor[df_surv_censor$BRAF_mut=="p.V600E",], "NOTCH_half", 
        main = "TCGA Set\n NOTCH_q\n BRAF mutatnt",height = 10, width=10, filename = "survival_NOTCH_half_mt_5yrs.png")


df_surv_censor <- df_surv %>% 
                mutate(vital_status = ifelse(days_to_death > 1095, "Alive", vital_status)) %>%
                mutate(days_to_death = ifelse(days_to_death > 1095, 1095, days_to_death))  %>%
                mutate(days_to_last_follow_up = ifelse(days_to_last_follow_up > 1095, 1095, days_to_last_follow_up)) %>%  
                mutate(days = ifelse(days > 1095, 1095, days))  


TCGAanalyze_survival(df_surv_censor[df_surv_censor$BRAF_mut=="WT",], "NOTCH_half", risk.table=FALSE, 
        main = "TCGA Set\n NOTCH_q\n BRAF mutatnt",height = 10, width=10, filename = "survival_NOTCH_half_wt_3yrs.png")

TCGAanalyze_survival(df_surv_censor[df_surv_censor$BRAF_mut=="p.V600E",], "NOTCH_half", 
        main = "TCGA Set\n NOTCH_q\n BRAF mutatnt",height = 10, width=10, filename = "survival_NOTCH_half_mt_3yrs.png")
